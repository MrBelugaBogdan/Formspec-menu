...
function checkAuth() {
  ...
  if (md5(keyFromUrl) === PASSPHRASE_HASH || md5(inputPass) === PASSPHRASE_HASH) {
    ...
    updateSidebarMenu();
  } else if (inputPass !== "") let currentVersion = 7;
let elements = [];
let selectedId = null;

const GRID_SCALE = 32; // Масштаб сітки редактора
...
function checkAuth() {
  ...
  if (typeof PASSPHRASE_HASH !== 'undefined' && (md5(keyFromUrl) === PASSPHRASE_HASH || md5(inputPass) === PASSPHRASE_HASH)) {
    ...
    // Якщо хеш не задано або авторизація не вимагається
    document.getElementById('auth-screen').style.display = 'none';
    document.getElementById('app').style.display = 'flex';
    updateSidebarMenu();
  }
...
function render() {
  ...
  elements.forEach(el => {
    ...
    // Враховуємо візуальний коефіцієнт сітки 1.25 у прев'ю редактора для слотів
    let displayW = el.w;
    let displayH = el.h;
    ...
    div.style.width = (displayW * GRID_SCALE) + 'px';
    div.style.height = (displayH * GRID_SCALE) + 'px';
...
function buildCmd() {
  ...
  // Обчислюємо точний розмір вікна Minetest з урахуванням того, що слоти займають 1.25x
  let maxW = 12.5;
  let maxH = 10;
  ...
  const cmdOutput = document.getElementById('cmd-result');
  if (cmdOutput) {
    if (typeof generateGivemeCommand === 'function') {
      cmdOutput.value = generateGivemeCommand(code);
    } else {
      cmdOutput.value = code;
    }
  }
}
